...
export function JobsPage() {
  ...
  const handleDeleteJob = async (jobId: number) => {
    const confirmed = window.confirm('Bu ilanı silmek istediğine emin misin?')
    if (!confirmed) {
      return
    }
    ...
    try {
      ...
      if (isAxiosError(err)) {
        ...
        setError(apiMessage ?? 'İlan silinemedi.')
      } else {
  ...
  const handleCloseJob = async (jobId: number) => {
    ...
    try {
      await jobsService.updateStatus(jobId, 'CLOSED')
      await loadJobs()
    } catch (err) {
      if (isAxiosError(err)) {
        const apiMessage =
          (err.response?.data as { error?: { message?: string } } | undefined)?.error?.message ??
          (err.response?.data as { message?: string } | undefined)?.message
        setError(apiMessage ?? 'İlan yayından kaldırılamadı.')
      } else {
        setError('İlan yayından kaldırılamadı.')
      }
    } finally {
                    ...
                    <div className="manage-row">
                      ...
                      <div className="delivery-actions">
                        ...
                        <button
                          ...
                          type="button"
                          disabled={closingJobId === job.id || (job.status ?? '').toUpperCase() !== 'OPEN'}
                          onClick={() => void handleCloseJob(job.id)}
                        >
                          {closingJobId === job.id ? 'Kapatılıyor...' : 'Yayından Kaldır'}
                        </button>
                        <button
                          className="button button-subtle"
                          type="button"
                        ...
                        >
                          {deletingJobId === job.id ? 'Siliniyor...' : 'İlanı Sil'}
                        </button>
                      </div>
                    </div>
                  </article>
